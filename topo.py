...
# Function to start streaming client
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Combined web and video streaming application.')
    parser.add_argument('--autotest', dest='autotest', action='store_const', const=True, default=False,
                        help='Enables automatic testing of the topology and closes the applications.')
    ...
    # Creating threads to run the servers and clients
    streaming_server_thread = threading.Thread(target=start_streaming_server)
    streaming_client_thread = threading.Thread(target=start_streaming_client)
    web_server_thread = threading.Thread(target=start_web_server)
    web_client_thread = threading.Thread(target=start_web_client)

    # Starting the threads
    streaming_server_thread.start()
    streaming_client_thread.start()
    web_server_thread.start()
    web_client_thread.start()

    # Waiting for the server and client threads to finish
    streaming_server_thread.join()
    streaming_client_thread.join()
    web_server_thread.join()
    web_client_thread.join()
